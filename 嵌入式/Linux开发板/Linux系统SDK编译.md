...
1.  **交叉编译工具链 (Cross-Compilation Toolchain)**：
    *   这是SDK中最核心的部分。它运行在**开发主机**（通常是x86电脑）上，但生成的代码是在**目标平台**（如ARM、MIPS、RISC-V）上运行的。
...
“Linux系统SDK编译”这个题目，本质上是讲述**如何利用自动化工具（如Buildroot/Yocto），从源代码构建出一个为特定嵌入式硬件定制的、高度集成和一致的开发环境（即SDK）的过程**。

你找到的这篇文档，就是通往这个环境的“操作手册”。遵循它，你可以获得一个强大且可靠的开发基础，从而更高效地开展后续的应用程序开发工作。