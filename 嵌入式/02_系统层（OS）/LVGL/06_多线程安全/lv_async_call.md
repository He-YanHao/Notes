...
lv_async_call 是 LVGL 图形库提供的一个核心工具函数，用于在图形库的主循环上下文中安全地执行一个回调函数。它允许开发者从任何线程、中断或回调中，将一个任务投递到 LVGL 的主任务中去执行，从而解决多任务环境下的线程安全问题。
...
-   async_xcb 是要执行的回调函数指针，类型为 void (*)(void*)。
...
此外，lv_async_call 本身并不是线程安全的。如果在非 LVGL 任务中调用它，而该任务与运行 lv_timer_handler 的任务之间没有加锁，可能会导致内部数据竞争。在多任务系统中，建议在调用 lv_async_call 时使用互斥锁进行保护。
...
lv_async_call 是多任务环境中操作 LVGL 界面的首选标准方法，它提供了简洁且高效的线程间通信机制。而 lv_async_call_cancel 则为其提供了更精细的控制能力，帮助开发者避免无效操作和内存问题。使用时务必小心处理 user_data 的分配与释放，并根据需要配合互斥锁保证调用安全。
